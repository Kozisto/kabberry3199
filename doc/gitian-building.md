...
*Setup instructions for a gitian build of Kabberry Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Kabberry
...
to the Kabberry GitHub release page.
...
- [Building Kabberry Core](#building-kabberry-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/kabberry-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/kabberry-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/kabberry-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/kabberry-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/kabberry-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/kabberry-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/kabberry-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@kabberry.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/kabberry-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/kabberry-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/kabberry-osx-*-build.assert
```
