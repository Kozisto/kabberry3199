...
public:
    ...
    bool WritesKKCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadsKKCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadsKKCSeed_deprecated(uint256& seed);
    bool ErasesKKCSeed();
    bool ErasesKKCSeed_deprecated();

    bool WritesKKCCount(const uint32_t& nCount);
    bool ReadsKKCCount(uint32_t& nCount);
