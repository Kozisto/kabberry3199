...
CsKKCWallet::CsKKCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadsKKCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasesKKCSeed_deprecated()) {
                    LogPrintf("%s: Updated sKKC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CsKKCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritesKKCCount(nCountLastUsed);
    else if (!walletdb.ReadsKKCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CsKKCWallet::Lock()
{
    seedMaster = 0;
}

void CsKKCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CsKKCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTosKKC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CsKKCWallet::LoadMintPoolFromDB()
{
...
void CsKKCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CsKKCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CsKKCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CsKKCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTosKKC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CsKKCWallet::SeedTosKKC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CsKKCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CsKKCWallet::GenerateDeterministicsKKC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CsKKCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CsKKCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CsKKCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
